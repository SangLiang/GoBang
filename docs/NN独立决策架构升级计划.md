...
| 里程碑 | 状态 | 开始日期 | 完成日期 | 备注 |
|--------|------|----------|----------|------|
| M1：特征提取模块 | ✅ 已完成 | 2026-03-31 | 2026-03-31 | 已完成 T1.1/T1.2/T1.3，并通过 `scripts/verify-features.js` 一致性校验 |
| M2：NN 决策接入 | ✅ 已完成 | 2026-03-31 | 2026-03-31 | 已完成 T2.1~T2.5，脚本冒烟验证通过 |
| M3：进化验证与调参 | ⬜ 未开始 | - | - | |
| M4：稳定化与优化 | ⬜ 未开始 | - | - | |
| M5：高级特性（可选） | ⬜ 未开始 | - | - | |
...
| 任务 | 状态 | 负责 | 备注 |
|------|------|------|------|
| T1.1 重写 nnFeatures.js | ✅ | AI Agent | 已新增 v2 22 维特征，保留 v1 默认接口兼容 |
| T1.2 boardCore.js 新增 buildLocalFeatures | ✅ | AI Agent | Node 侧实现完成，并与浏览器侧逻辑对齐 |
| T1.3 verify-features.js 验证脚本 | ✅ | AI Agent | 已新增并通过 3 组场景校验（维度/范围/一致性） |

#### M2：NN 决策接入

| 任务 | 状态 | 负责 | 备注 |
|------|------|------|------|
| T2.1 重写 AI.js 评分逻辑 | ✅ | AI Agent | 已新增 v2 单列表评分分支，保留 v1/rule 兼容路径 |
| T2.2 重写 nnAssistPick.js | ✅ | AI Agent | 已切换到 `buildLocalFeatures` + `lambda*score` 单列表选点 |
| T2.3 更新 evolve-ai.js | ✅ | AI Agent | 网络改为 `[22,[32],1]`，输出 `nnAssistSchemaVersion=2` |
| T2.4 更新 nnAssist.js | ✅ | AI Agent | 已支持 v1/v2 双网络加载，新增 `computeV2` |
| T2.5 更新 config.js | ✅ | AI Agent | 已设置 `NN_ASSIST_SCHEMA_VERSION=2`、`NN_LAMBDA=1000` |

#### M3：进化验证与调参

| 任务 | 状态 | 负责 | 备注 |
|------|------|------|------|
| T3.1 阶段 1 进化（可行性） | ✅ | AI Agent | 已运行 v2-exp-001；bestClean=-1.0000，20 代无提升 |
| T3.2 阶段 2 进化（突破上限） | ✅ | AI Agent | 已运行 v2-exp-002；bestCleanEver=1.0378，显著突破 0.58 |
| T3.3 阶段 3 进化（深度优化） | ⬜ | | |
...
|----------|------|-----------|------|
| v2-exp-001 | POP=50, GEN=20, λ=500 | -1.0000 | 已运行；clean[min/med/max] 全程 -1.0000，建议进入参数排查 |
| v2-exp-001a | POP=50, GEN=10, GAMES=8, λ=1000 | 0.0189 | 已运行；第 3 代起出现正向个体，证明链路可学习 |
| v2-exp-001b | POP=50, GEN=10, GAMES=8, λ=1500 | 0.0189 | 已运行；第 1 代起出现正向个体，分布较 λ=1000 略更快拉开 |
| v2-exp-002 | POP=100, GEN=50, GAMES=8, λ=1000 | 1.0378 | 已运行；第 5 代跃迁到 1.0376+，最终 bestCleanEver=1.0378 |
